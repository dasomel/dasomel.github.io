...
This is an introductory piece walking through the common challenges of implementing runtime security for cloud-native workloads and how to get started overcoming them with Falco, the open-source runtime threat detection tool. Falco works by watching what each individual container is actually doing and flagging suspicious behavior for further investigation. For example, a rule can flag something like a redis container opening an outbound network connection it shouldn't. Plugins let Falco ingest not just Kubernetes and cloud audit logs but also events from SaaS services like GitLab, Box, or Salesforce, with Falcosidekick routing triggered alerts out to channels such as Slack, Prometheus, and PagerDuty. Full access to the article body was blocked, so this summary is based on the title, excerpt, and related public information rather than a direct read of the full text.
...
Stripe published data on its blog arguing that SaaS platform businesses are actually surging, not shrinking, in the aftermath of what the press dubbed the "SaaSpocalypse". In late January, software companies shed roughly $1 trillion in combined market capitalization over 30 days as investors worried that agentic AI would commoditize software. Stripe's take is that while that was a useful warning shot for the industry, SaaS platforms that run businesses' core operations have only become more deeply embedded since. As evidence, Stripe points to new platform businesses launched on its own infrastructure being up 182% year over year. The company frames this as platforms increasingly handling money movement on top of core workflows, which makes them harder to commoditize away.
