...
OpenAI가 ChatGPT에 Health 기능을 출시했다. 자격을 갖춘 미국 사용자를 대상으로 시작한다고 밝혔다.

> 💡 의료 영역으로 소비자 AI가 들어오면 규제 경계가 즉시 쟁점이 되므로, 의료 데이터를 다루는 서비스라면 사용자 기대치 변화와 함께 준수 요건을 점검해둘 필요가 있다.
...
The New Stack이 OpenAI와 앤스로픽이 같은 목요일 오후에 각각 주요 음성 기능 업데이트를 내놓은 상황을 다뤘다. 두 프런티어 AI 연구소가 동시에 음성 쪽에 힘을 실었지만 서로 다른 방향을 보고 있다는 것이 기사의 구도다.

> 💡 음성 인터페이스가 동시에 강화된다는 것은 텍스트 중심으로 설계한 에이전트 워크플로에 음성 진입점을 어떻게 붙일지 검토할 시점이라는 신호다.

### [Nvidia’s new DNA model learns what token prediction misses](https://thenewstack.io/nvidia-jepa-dna-genomics/)

_The New Stack_

The New Stack이 NVIDIA의 새 DNA 모델을 다뤘다. AI 업계가 대체로 언어 기반 접근에 집중해 방대한 데이터셋으로 학습한 트랜스포머로 단어를 예측하거나 빈칸을 채우는 방식을 써왔다는 것이 배경이다. 제목이 밝히듯 이 DNA 모델은 토큰 예측이 놓치는 것을 학습한다는 점이 초점이며, JEPA 계열 접근을 유전체학에 적용한 사례로 다뤄진다.

> 💡 토큰 예측이 아닌 표현 학습 목표가 도메인에 따라 더 맞을 수 있다는 사례라, 시퀀스 데이터를 다루는 문제에서 언어 모델 방식이 기본값인지 되짚어볼 만하다.
...
The New Stack이 NVIDIA가 로컬 모델과 프런티어 모델을 어떻게 보는지 다뤘다. 책상 위 장비에서 돌아갈 만큼 작은 모델들이 충분히 좋아지고 있어 흥미로운 질문이 달라지고 있다는 것이 배경이다. 제목에 인용된 "둘 다 쓸 수 있는 세상을 우리는 좋아한다"는 표현이 NVIDIA의 입장을 요약한다.

> 💡 로컬과 프런티어를 배타적 선택으로 보지 않는 관점은, 작업 민감도와 지연 요건에 따라 모델을 갈라 태우는 라우팅 설계를 전제로 한다.
