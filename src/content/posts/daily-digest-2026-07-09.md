...
OpenAI가 정부·국가 안보 파트너십에 대한 접근 방식을 공개했다. 책임 있는 AI 사용, 민주적 책임성, 공공 안전을 원칙으로 삼는다고 설명한다.

> 💡 공공 부문 도입을 검토 중이라면 벤더가 명시한 사용 원칙이 조달 요건과 충돌하는 지점을 계약 전에 확인해둘 필요가 있다.
...
OpenAI가 코딩 평가에서 신호와 잡음을 분리하는 분석을 내놨다. 널리 쓰이는 코딩 벤치마크 SWE-Bench Pro에서 문제점을 발견했다는 내용으로, AI 모델 평가의 신뢰성과 정확성에 대한 우려를 제기한다.

> 💡 공개 코딩 벤치마크 점수로 모델을 고르고 있다면, 벤치마크 자체의 결함 가능성 때문에 자사 저장소 기반의 소규모 평가 세트를 따로 두는 편이 안전하다.

### [Helping K–12 educators build practical AI skills](https://openai.com/index/k-12-educators-practical-skills)

_OpenAI_

OpenAI Academy와 월턴 패밀리 재단이 K-12 교사를 위한 실습형 AI Skills Jams를 진행한다고 밝혔다. 교사들이 교실에서 쓸 수 있는 실용적 AI 역량을 기르도록 돕는 것이 목적이다.

> 💡 교육 현장 대상 프로그램은 도구 사용법보다 검증 습관을 어떻게 가르치느냐가 실제 성과를 가르므로, 유사 사내 교육을 설계할 때도 같은 기준이 적용된다.
...
The New Stack이 Grok 4.5 공개 소식을 다뤘다. SpaceXAI CEO 일론 머스크가 수요일에 Grok 4.5를 목요일 공개한다고 X에 올렸다고 전한다. 기사 제목은 머스크가 앤스로픽을 이기는 것에 대해 한 말, 즉 "Opus급이지만 더 빠르다"는 표현을 인용한다.

> 💡 벤더가 내세우는 "동급 성능에 더 빠름"은 자사 워크로드에서 재현되지 않는 경우가 많으므로, 전환을 검토한다면 지연과 품질을 같은 프롬프트로 직접 측정해야 한다.
...
The New Stack이 JetBrains의 다음 수가 더 나은 IDE가 아니라 Claude Code·Codex·Gemini CLI 위에 얹는 거버넌스 계층이라고 전했다. 엔지니어링 팀들이 지난 몇 년간 각자 AI 도구를 골라 써왔다는 것이 배경이다. IDE 하나, 터미널 기반 코딩 도구 하나 식으로 파편화된 상황을 겨냥한다는 것이 기사의 논지다.

> 💡 팀마다 다른 코딩 에이전트를 쓰고 있다면 도구 통일보다 정책·감사 계층을 공통화하는 쪽이 현실적인 수습 방향일 수 있다.

### [Meta says it caught OpenAI. One thing is missing.](https://thenewstack.io/meta-watermelon-benchmark-claim/)

_The New Stack_

The New Stack이 메타가 OpenAI를 따라잡았다는 주장을 검증했다. 마크 저커버그가 메타 직원들에게 회사의 AI 베팅이 "아직 결실을 맺지 못했다"고 말한 바로 그 주에, 그의 초지능 조직에서 벤치마크 관련 주장이 나왔다는 것이 기사의 구도다. 제목이 밝히듯 그 주장에는 한 가지가 빠져 있다는 것이 논지다.

> 💡 벤더 자체 벤치마크 발표는 재현 조건이 빠져 있는 경우가 잦으므로, 모델 도입 판단에 쓰기 전에 평가 설정이 공개됐는지부터 확인하는 편이 안전하다.
