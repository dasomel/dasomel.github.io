...
When Pod(openbao) is in not ready status after restart:

> Since v2.2.0, OpenBao unseal key automatic management has been added, so manual unseal is not required.

```shell
# For manual unseal if needed
