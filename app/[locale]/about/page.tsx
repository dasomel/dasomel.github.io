...
export default async function AboutPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12 sm:py-16 slide-enter-content">
    ...
    <section className="mb-16"><SectionHeader icon={Wrench} title={isEn ? 'What I Build' : '무엇을 만드는가'} /><div className="grid md:grid-cols-2 gap-3">{projects.slice(0, 6).map(project => <a key={project.slug} href={`/${lang}/projects/${project.slug}`} className="rounded-2xl p-5 group transition-all hover:bg-[var(--surface-hi)]" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}><div className="flex items-center justify-between gap-3 mb-2"><h3 className="font-semibold group-hover:text-[var(--accent)] transition-colors" style={{ color: 'var(--text)' }}>{project.title}</h3><span className="text-xs font-mono" style={{ color: 'var(--text-faint)' }}>{project.tags[0]}</span></div><p className="text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>{project.description}</p></a>)}</div></section>
    ...
    <section className="mb-16"><SectionHeader icon={Briefcase} title={isEn ? 'Roles' : '역할'} /><div className="space-y-3">{roles.map((r, i) => <div key={i} className="p-5 rounded-xl" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}><div className="flex items-start justify-between gap-4 mb-2"><div><h3 className="font-bold" style={{ color: 'var(--text)' }}>{r.icon} {r.role}</h3>{'org' in r && r.org && <p className="text-sm font-medium" style={{ color: 'var(--accent)' }}>{r.org}</p>}</div><span className="font-mono text-xs whitespace-nowrap" style={{ color: 'var(--text-faint)' }}>{r.period}</span></div><p className="text-sm" style={{ color: 'var(--text-muted)' }}>{r.desc}</p></div>)}</div></section>

    <section className="grid lg:grid-cols-2 gap-8 mb-16">
      <div><SectionHeader icon={Globe} title={isEn ? 'Community' : '커뮤니티'} /><div className="space-y-3">{communityActivities.slice(0, 6).map((a, i) => <div key={i} className="p-4 rounded-xl" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-semibold" style={{ color: 'var(--text)' }}>{a.org}</div><div className="text-xs mt-1" style={{ color: 'var(--accent)' }}>{a.role}</div></div><span className="text-xs font-mono" style={{ color: 'var(--text-faint)' }}>{a.period}</span></div><p className="text-xs mt-2 leading-relaxed" style={{ color: 'var(--text-muted)' }}>{a.desc}</p></div>)}</div></div>
      <div><SectionHeader icon={Code} title={isEn ? 'Open Source Contributions' : '오픈소스 기여'} /><div className="space-y-3">{contributions.map(c => <a key={c.name} href={c.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between gap-3 p-4 rounded-xl group transition-all hover:bg-[var(--surface-hi)]" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}><div><div className="font-semibold group-hover:text-[var(--accent)] transition-colors" style={{ color: 'var(--text)' }}>{c.name}</div><p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>{c.desc}</p></div><ExternalLink className="w-4 h-4" style={{ color: 'var(--text-faint)' }} /></a>)}</div></div>
    </section>
