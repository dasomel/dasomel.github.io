...
export default async function BlogPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params; const lang = locale as 'ko' | 'en'; const t = await getTranslations({ locale, namespace: 'notes' }); const posts = getNotes(lang); const base = lang === 'en' ? '/en' : '/ko';
  ...
  return <div className="max-w-4xl mx-auto px-4 sm:px-6 py-16 slide-enter-content"><div className="workbench-eyebrow mb-3">NOTES</div><h1 className="text-3xl font-bold mb-2" style={{ color: 'var(--text)' }}>{t('title')}</h1><p className="text-base mb-8" style={{ color: 'var(--text-muted)' }}>{t('subtitle')}</p><PostList posts={postsWithReadTime} base={base} translations={{ featured: t('featured'), all_posts: t('all_posts') }} /></div>;
}
