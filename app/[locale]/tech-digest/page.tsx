...
export default async function TechDigestPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <div className="max-w-4xl mx-auto px-4 sm:px-6 py-16 slide-enter-content"><div className="flex items-start justify-between gap-4 mb-3"><div><div className="workbench-eyebrow mb-2">TECH DIGEST</div><h1 className="text-3xl font-bold" style={{ color: 'var(--text)' }}>{t('title')}</h1></div><Link href={rss} className="inline-flex items-center gap-2 text-xs font-mono px-3 py-2 rounded-lg" style={{ color: 'var(--text-muted)', border: '1px solid var(--border)' }}><Rss className="w-3.5 h-3.5" aria-hidden="true" />RSS</Link></div><p className="text-base mb-8" style={{ color: 'var(--text-muted)' }}>{t('subtitle')}</p><div className="rounded-2xl p-4 mb-8" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}><p className="text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>{t('notice')}</p></div><PostList posts={postsWithReadTime} base={`${base}`} showTagFilter={false} translations={{ featured: t('featured'), all_posts: t('all_posts') }} /></div>;
}
