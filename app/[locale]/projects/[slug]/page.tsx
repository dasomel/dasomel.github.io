  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
      ...
      <header className="mb-10">
        <div className="overflow-hidden rounded-2xl mb-7" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}><img src={image} alt="" className="block w-full h-auto" loading="eager" /></div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {meta.github && <Button asChild size="sm"><a href={meta.github} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2"><Github className="w-4 h-4" />{tc('github')}<ArrowUpRight className="w-3 h-3" /></a></Button>}
          {meta.tags.map(tag => <Badge key={tag}>{tag}</Badge>)}
        </div>
      </header>
      {(meta.problem || meta.solution) && <section className="grid md:grid-cols-2 gap-3 mb-10">
