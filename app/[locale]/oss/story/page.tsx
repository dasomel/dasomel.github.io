    ...
    <main>
      ...
      <section className="border-b" style={{borderColor:'var(--border)'}}>
        <div className="mx-auto max-w-[1440px] px-5 py-12 sm:px-8 lg:px-16 lg:py-16">
          ...
          <div className="mt-7 flex flex-wrap items-center gap-3 rounded-2xl p-4 sm:max-w-4xl" style={{border:'1px solid var(--border)',backgroundColor:'var(--surface-hi)'}}><span className="font-mono text-[10px] font-semibold uppercase tracking-[0.12em]" style={{color:'var(--signal)'}}>{en?'CONTEXT':'CONTEXT'}</span><span className="text-sm" style={{color:'var(--text-muted)'}}>{en?'Looking for the rationale behind the OSS practice rather than the personal evolution?':'개인의 변화 과정보다 OSS를 만드는 이유와 운영 원칙이 궁금하다면'}</span><Link href={`/${currentLocale}/oss/why/`} className="text-sm font-semibold" style={{color:'var(--accent)'}}>{en?'Why OSS →':'Why OSS →'}</Link></div>
        </div>
      </section>
