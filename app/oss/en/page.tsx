  ...
  return (
    ...
    └── KubeMetal ──→ Apple Silicon / host-native MLX MLOps`}</pre>
              ...
              {group.projects.map((slug) => {
                ...
                return (
                  <Link key={slug} href={`/oss/en/${slug}/`} className="group rounded-2xl p-7 transition duration-200" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}>
                    ...
                    <div className="mt-7 text-sm font-medium" style={{ color: 'var(--accent)' }}>View project documentation →</div>
                  </Link>
                );
              })}
            </div>
      ...
      <section className="mt-16 border-t pt-10" style={{ borderColor: 'var(--border)' }}>
        <h2 className="text-2xl font-semibold">Documentation Standard</h2>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href="/oss/openforge/" className="rounded-full px-4 py-2 text-sm font-medium" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)', color: 'var(--text)' }}>OpenForge documentation baseline →</Link>
          <Link href="/projects/" className="rounded-full px-4 py-2 text-sm font-medium" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)', color: 'var(--text)' }}>All Projects →</Link>
        </div>
      </section>
    </div>
  );
...
function Metric({ label, value, note, wide = false }: { label: string; value: string; note: string; wide?: boolean }) {
  return (
    <div className="rounded-2xl p-5" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}>
      <div className="text-xs uppercase tracking-[0.14em]" style={{ color: 'var(--text-faint)' }}>{label}</div>
      <div className={wide ? 'mt-2 text-xl font-semibold' : 'mt-2 text-3xl font-semibold'}>{value}</div>
      <div className="mt-1 text-sm" style={{ color: 'var(--text-muted)' }}>{note}</div>
    </div>
  );
}
