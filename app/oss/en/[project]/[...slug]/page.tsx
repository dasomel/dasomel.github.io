...
export default async function OssDocPageEn({ params }: { params: Promise<{ project: string; slug: string[] }> }) {
  ...
  return <OssProjectShell project={project.meta} docs={docs} locale="en"><div className="mb-10"><div className="mb-3 font-mono text-xs text-slate-500">/oss/en/{projectSlug}/{segments.join('/')}</div><h2 className="text-4xl font-bold tracking-tight">{doc.meta.title}</h2>{doc.meta.description && <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-300">{doc.meta.description}</p>}</div><article className="prose prose-invert max-w-3xl prose-headings:scroll-mt-24"><MDXContent source={doc.content} /></article></OssProjectShell>;
}
