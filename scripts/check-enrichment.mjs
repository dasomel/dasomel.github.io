...
const enriched = articles.filter((a) =>
  ...
  a.insightKo && String(a.insightKo).trim() &&
  a.insightEn && String(a.insightEn).trim()
);
const detailed = articles.filter((a) =>
  enriched.includes(a) && detailedEnough(a.summaryKo) && detailedEnough(a.summaryEn)
);
const pct = Math.round((enriched.length / articles.length) * 100);
const detailedPct = Math.round((detailed.length / articles.length) * 100);

if (enriched.length === 0) {
  if (strict) fail(`${DATE} AI 보강이 하나도 없다 (0/${articles.length}건). 자동 발행을 중단한다.`);
  warn(`${DATE} 다이제스트가 보강 없이 발행됐다.`);
  process.exit(0);
}
...
if (fs.existsSync(KO)) {
  ...
  if (needle.length >= 20 && !haystack.includes(needle)) {
    if (strict) fail(`${DATE}: JSON에는 AI 보강이 있지만 ko 마크다운에 반영되지 않았다. 마크다운을 JSON에서 재생성하라.`);
    warn(`${DATE}: JSON에는 AI 보강이 있지만 ko 마크다운에 반영되지 않았다. 마크다운을 JSON에서 재생성하라.`);
    process.exit(0);
  }
}

if (detailed.length < articles.length) {
  warn(`${DATE} 상세 보강이 부분적이다: ${detailed.length}/${articles.length}건 (${detailedPct}%). 자동 발행은 차단한다.`);
  process.exit(0);
}
console.log(`✓ ${DATE} 상세 AI 보강 완료 — ${detailed.length}/${articles.length}건 (100%)`);
