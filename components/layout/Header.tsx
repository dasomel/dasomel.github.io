...
export default function Header({ locale }: HeaderProps) {
  ...
  const work = { href: `${base}/projects`, label: 'Work', aliases: [`${base}/projects`] };
  const oss = { href: ossHref, label: 'OSS', aliases: ['/oss', `${base}/oss`] };
  const knowledge = { href: `${base}/knowledge`, label: 'Knowledge', aliases: [`${base}/knowledge`, `${base}/notes`, `${base}/posts`, `${base}/seminars`, `${base}/docs`, `${base}/events`] };
  // D1: Keep Tech Digest first-level for PC discoverability; cost: one nav slot; escape hatch: move it under Knowledge if widths regress.
  const techDigest = { href: `${base}/tech-digest`, label: t('tech_digest'), aliases: [`${base}/tech-digest`] };
  const about = { href: `${base}/about`, label: t('about'), aliases: [`${base}/about`] };
  const primary = [
    work,
    oss,
    knowledge,
  ...
  const desktopPrimary = [work, oss, knowledge, techDigest, about];
  const mobileExtra = [
      ...
      <div className="mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-16">
        <div className="flex h-14 items-center justify-between sm:h-16">
          <Link href={`${base}/`} className={`inline-flex items-center gap-2.5 ${focusRing}`} aria-label="dasomel Open Engineering Lab">
            ...
            {/* D2: Hide the long tagline at medium widths to keep the expanded desktop nav on one row; cost: less brand detail at 768–1279px; escape hatch: restore it if the nav is shortened. */}
            <span className="hidden font-mono text-[10px] tracking-[0.12em] sm:inline md:hidden xl:inline" style={{color:'var(--text-faint)'}}>/ OPEN ENGINEERING LAB</span>
          </Link>
          <nav className="hidden items-center gap-1 md:flex" aria-label="Primary navigation">
            {desktopPrimary.map(item => { const active=itemActive(item); return <Link key={item.href} href={item.href} className={`rounded-lg px-3 py-1.5 text-sm transition-colors ${focusRing}`} style={{color:active?'var(--accent)':'var(--text-muted)',fontWeight:active?600:400}}>{item.label}</Link>; })}
