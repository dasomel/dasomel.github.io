    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6">
      <div className="flex items-center justify-between h-14 sm:h-16">
        ...
        <nav className="hidden md:flex items-center gap-1">
          ...
          <ThemeToggle />
          <Link href={langSwitch} className={`px-2.5 py-1 font-mono text-xs rounded-lg border ${focusRing}`} style={{ color:'var(--text-muted)', borderColor:'var(--border)' }}>{t('lang')}</Link>
        </nav>

        <div className="flex items-center gap-2 md:hidden">
          <ThemeToggle />
          <Link href={langSwitch} className={`font-mono text-xs px-2.5 py-1.5 border rounded-md ${focusRing}`} style={{ color:'var(--text-muted)', borderColor:'var(--border)' }}>{t('lang')}</Link>
          <button onClick={() => setOpen(!open)} className={`p-2 rounded-lg transition-colors ${focusRing}`} style={{ color:'var(--text-muted)', backgroundColor:open ? 'var(--surface-hi)' : 'transparent' }} aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open} aria-controls="mobile-navigation">
