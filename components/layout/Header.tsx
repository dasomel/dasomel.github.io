      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-14">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navItems.map(item => (
              <Link key={item.href} href={item.href} className="px-3 py-1.5 text-sm rounded-lg transition-colors" style={{ color: isActive(item.href) ? 'var(--text)' : 'var(--text-muted)', fontWeight: isActive(item.href) ? 600 : 400, backgroundColor: isActive(item.href) ? 'var(--surface-hi)' : 'transparent' }}>
                {item.label}
              </Link>
            ))}
