...
export function OssPortfolioMatrix({ locale='ko' }: { locale?:'ko'|'en' }) {
  ...
  return <section className="mt-16">
    ...
    <h2 className="mt-3 text-3xl font-semibold">{en?'See role, maturity, verification and coupling boundaries at a glance.':'역할·성숙도·검증 경계를 한 화면에서 봅니다.'}</h2>
    <p className="mt-3 max-w-4xl text-sm leading-7" style={{ color:'var(--text-muted)' }}>{en?'The matrix avoids synthetic scores and uses states that can be grounded in current project documentation. Independence distinguishes shared ecosystem context from mandatory runtime coupling.':'숫자 점수 대신 현재 문서와 저장소에서 확인 가능한 상태를 사용합니다. 특히 독립성은 “같은 생태계에 보인다”와 “런타임으로 결합된다”를 구분하기 위한 항목입니다.'}</p>
