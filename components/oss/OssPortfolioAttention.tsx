...
export function OssPortfolioAttention({ locale, docs }: Props) {
  ...
  return <section className="mt-12">
    ...
    <p className="mt-3 max-w-4xl text-sm leading-7" style={{ color:'var(--text-muted)' }}>{en?'These are factual maintenance signals derived from repository and documentation snapshots, not severity scores. Existing tracker links now include their live issue state; a closed tracker is history, not proof that the current signal is resolved.':'저장소·문서 snapshot에서 확인되는 사실 기반 유지보수 신호입니다. 기존 tracker는 실제 issue 상태도 함께 표시하며, tracker가 closed라고 해서 현재 signal이 해결됐다고 자동 판단하지 않습니다.'}</p>
