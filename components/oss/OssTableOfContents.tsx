        ...
        <nav className="mt-4 space-y-1" aria-label={title}>
          {items.map((item) => (
            <Link
              key={item.id}
              href={`#${item.id}`}
              className={`block border-l-2 py-1.5 text-xs leading-5 transition ${
                item.level === 3
                  ? 'ml-3 border-transparent pl-3 text-[#8a8a82] hover:border-[#5b8c87] hover:text-[#34342f]'
                  : 'border-transparent pl-3 text-[#5c5c55] hover:border-[#0f766e] hover:text-[#171717]'
              }`}
            >
              {item.label}
            </Link>
