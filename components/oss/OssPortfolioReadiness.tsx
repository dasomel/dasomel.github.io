...
type Gap = { slug:string; name:string; kind:ReadinessGapKind; label:string; detail:string; tracker?:OssTracker };
...
export function OssPortfolioReadiness({ locale, docs }:Props) {
  ...
  for (const project of OSS_PORTFOLIO_PROJECTS) {
    ...
    if (!evidencePresent(evidence?.license)) gaps.push({ slug,name,kind:'license',label:en?'LICENSE evidence missing':'LICENSE 근거 없음',detail:en?'No root/.github LICENSE evidence was observed by the repository audit.':'repository audit에서 root/.github LICENSE 근거를 확인하지 못했습니다.',tracker:getReadinessTracker(slug,'license') });
    if (!evidencePresent(evidence?.contributing)) gaps.push({ slug,name,kind:'contributing',label:en?'CONTRIBUTING evidence missing':'CONTRIBUTING 근거 없음',detail:en?'No root/.github CONTRIBUTING evidence was observed.':'root/.github CONTRIBUTING 근거를 확인하지 못했습니다.',tracker:getReadinessTracker(slug,'contributing') });
    if (!evidencePresent(evidence?.security)) gaps.push({ slug,name,kind:'security',label:en?'SECURITY policy evidence missing':'SECURITY 정책 근거 없음',detail:en?'No root/.github SECURITY evidence was observed.':'root/.github SECURITY 근거를 확인하지 못했습니다.',tracker:getReadinessTracker(slug,'security') });
    if (!evidencePresent(evidence?.sbom)) gaps.push({ slug,name,kind:'sbom',label:en?'SBOM evidence missing':'SBOM 근거 없음',detail:en?'No SBOM/SPDX/CycloneDX evidence path was observed in the repository tree.':'repository tree에서 SBOM/SPDX/CycloneDX 근거 경로를 확인하지 못했습니다.',tracker:getReadinessTracker(slug,'sbom') });
    if (!evidencePresent(evidence?.provenance)) gaps.push({ slug,name,kind:'provenance',label:en?'Provenance evidence missing':'Provenance 근거 없음',detail:en?'No provenance/attestation/SLSA/cosign evidence path was observed.':'provenance/attestation/SLSA/cosign 근거 경로를 확인하지 못했습니다.',tracker:getReadinessTracker(slug,'provenance') });
    if ((meta?.releaseCount??0)===0) gaps.push({ slug,name,kind:'release',label:en?'No GitHub release':'GitHub Release 없음',detail:en?'The build-time GitHub snapshot reports zero releases.':'build-time GitHub snapshot 기준 release가 0개입니다.',tracker:getReadinessTracker(slug,'release') });
    if (docsCount===0) gaps.push({ slug,name,kind:'docs',label:en?'No indexed project docs':'프로젝트 문서 미인덱싱',detail:en?'No project detail documentation section is indexed on the OSS site.':'OSS 사이트에 프로젝트 상세 문서 section이 인덱싱되지 않았습니다.',tracker:getReadinessTracker(slug,'docs') });
    if ((meta?.contributorCount??0)<=1) gaps.push({ slug,name,kind:'community',label:en?'Single-contributor signal':'단일 contributor 신호',detail:en?'GitHub reports one or fewer contributors; this is an operating signal, not proof of no outside participation.':'GitHub contributor 집계가 1명 이하입니다. 외부 참여가 없다는 증거가 아니라 운영 신호입니다.',tracker:getReadinessTracker(slug,'community') });
  }
  ...
  return <section className="mt-16">
    ...
    <p className="mt-3 max-w-4xl text-sm leading-7" style={{ color:'var(--text-muted)' }}>{en?'Governance and security come from the repository tree audit, release and community from the GitHub snapshot, docs from the site index, and adoption only from an explicit evidence registry. Tracker state is observed independently and never overrides current repository evidence.':'Governance·Security는 repository tree audit, Release·Community는 GitHub snapshot, Docs는 사이트 index, Adoption은 명시적 evidence registry만 사용합니다. Tracker 상태는 별도 관찰 신호이며 현재 repository evidence를 덮어쓰지 않습니다.'}</p>
    ...
    <p className="mt-3 max-w-4xl text-sm leading-7" style={{ color:'var(--text-muted)' }}>{en?'Each item is generated from explicit audit or snapshot absence. Tracker links prefer project-specific issues when already known and otherwise point to the nearest canonical portfolio owner. Their current issue state is shown, but closing a tracker never suppresses a still-observed gap.':'각 항목은 audit 또는 snapshot에서 명시적으로 확인되지 않은 근거로부터 생성됩니다. 알려진 project-specific issue를 우선 연결하고 없으면 canonical owner를 안내하며, issue 상태도 표시합니다. 다만 tracker가 닫혀도 실제 gap이 관찰되면 숨기지 않습니다.'}</p>
    ...
    {visibleGaps.length>0&&<div className="mt-5 grid gap-3 md:grid-cols-2">{visibleGaps.map((gap)=>{const trackerState=gap.tracker?issueStateLabel(getOssIssueMetaByUrl(gap.tracker.url)):undefined;return <div key={`${gap.slug}-${gap.kind}`} className="rounded-2xl p-5" style={{ border:'1px solid var(--border)', backgroundColor:'var(--surface)' }}><div className="flex items-start justify-between gap-4"><div><div className="font-mono text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ color:'var(--signal)' }}>{gap.label}</div><div className="mt-2 text-lg font-semibold">{gap.name}</div></div><span className="rounded-full px-2.5 py-1 font-mono text-[10px]" style={{ border:'1px solid var(--border)', color:'var(--text-faint)' }}>{gap.kind}</span></div><p className="mt-3 text-sm leading-6" style={{ color:'var(--text-muted)' }}>{gap.detail}</p><div className="mt-4 flex flex-wrap gap-2"><Link href={`${prefix}/${gap.slug}/`} className="text-xs font-semibold" style={{ color:'var(--accent)' }}>{en?'Project context →':'프로젝트 맥락 →'}</Link>{gap.tracker&&<a href={gap.tracker.url} target="_blank" rel="noreferrer" className="rounded-full px-2.5 py-1 font-mono text-[10px]" style={{ border:'1px solid var(--accent)', color:'var(--accent)' }}>{gap.tracker.label} · {gap.tracker.source} · {trackerState} ↗</a>}</div></div>;})}</div>}

    <p className="mt-3 text-xs leading-5" style={{ color:'var(--text-faint)' }}>{en?'Audit matching is intentionally conservative: governance policy files are counted only at the repository root or under .github; SBOM and provenance evidence can live elsewhere in the tree. Adoption is equally conservative and changes only when an explicit source-backed record is added to the adoption registry.':'Audit 매칭은 보수적으로 적용합니다. governance 정책 파일은 저장소 root 또는 .github 아래만 인정하고, SBOM·provenance evidence는 tree 내 다른 위치도 탐색합니다. Adoption도 동일하게 보수적으로 적용하며 명시적 source가 있는 기록이 adoption registry에 추가될 때만 상태가 변경됩니다.'}</p>
  </section>;
}
