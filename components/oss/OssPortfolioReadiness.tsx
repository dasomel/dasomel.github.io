...
export function OssPortfolioReadiness({ locale, docs }:Props) {
  ...
  return <section className="mt-16">
    ...
    <p className="mt-3 max-w-4xl text-sm leading-7" style={{ color:'var(--text-muted)' }}>{en?'Governance and security come from the repository tree audit, release and community from the GitHub snapshot, docs from the site index, and adoption only from an explicit evidence registry. File presence and repository popularity are not treated as proof of effective controls or real-world adoption.':'Governance·Security는 repository tree audit, Release·Community는 GitHub snapshot, Docs는 사이트 index, Adoption은 명시적 evidence registry만 사용합니다. 파일 존재나 저장소 인기 지표를 정책 효과 또는 실제 adoption 증거로 간주하지 않습니다.'}</p>
