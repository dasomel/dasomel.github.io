...
export function ProjectList({ projects, base, translations }: Props) {
  ...
  const renderFeatured = (items: Project[]) => <div className="space-y-5 mb-9">{items.map(project => {
    ...
    return <article key={project.slug} className="overflow-hidden rounded-2xl transition-all hover:border-[var(--border-hi)]" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}>
      ...
      <div className="p-5 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <Link href={`${base}/projects/${project.slug}`}><h3 className="text-lg font-bold hover:text-[var(--accent)] transition-colors" style={{ color: 'var(--text)' }}>{project.title}</h3></Link>
          <div className="flex items-center gap-2"><StatusBadge status={status} /><span className="text-[10px] px-2 py-0.5 rounded-full" style={{ backgroundColor: project.type === 'fork' ? 'var(--bg-subtle)' : 'var(--surface-hi)', border: '1px solid var(--border)', color: project.type === 'fork' ? 'var(--accent)' : 'var(--text-muted)' }}>{project.type === 'fork' ? 'Fork' : 'Project'}</span>{project.github && <a href={project.github} target="_blank" rel="noopener noreferrer" className="p-1 hover:opacity-60 transition-opacity" style={{ color: 'var(--text-faint)' }} aria-label={`${project.title} GitHub`}><Github className="w-4 h-4" /></a>}</div>
        </div>
        {(project.problem && project.solution) ? <div className="grid md:grid-cols-2 gap-4 mb-4 text-sm"><div className="rounded-xl p-4" style={{ backgroundColor: 'var(--bg-subtle)' }}><div className="font-semibold mb-1" style={{ color: 'var(--text-faint)' }}>{translations.problem}</div><p style={{ color: 'var(--text-muted)' }} className="leading-relaxed">{project.problem}</p></div><div className="rounded-xl p-4" style={{ backgroundColor: 'var(--accent-dim)' }}><div className="font-semibold mb-1" style={{ color: 'var(--accent)' }}>{translations.solution}</div><p style={{ color: 'var(--text-muted)' }} className="leading-relaxed">{project.solution}</p></div></div> : <p className="text-sm leading-relaxed mb-4" style={{ color: 'var(--text-muted)' }}>{project.description}</p>}
        <div className="flex flex-wrap gap-1.5">{project.tags.map(tag => <span key={tag} className="px-1.5 py-0.5 text-xs font-mono rounded" style={{ backgroundColor: 'var(--surface-hi)', border: '1px solid var(--border)', color: 'var(--text-faint)' }}>{tag}</span>)}</div>
      </div>
    </article>;
  })}</div>;

  const renderCompact = (items: Project[]) => <div className="grid sm:grid-cols-2 gap-3">{items.map(project => { const status = resolveStatus(project); return <Link key={project.slug} href={`${base}/projects/${project.slug}`} className="group overflow-hidden rounded-2xl transition-all hover:border-[var(--border-hi)] hover:bg-[var(--surface-hi)]" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--surface)' }}><ProjectVisual src={safeImage(project.slug)} alt="" className="block w-full h-auto" loading="lazy" /><div className="p-4 flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2 mb-1"><div className="text-sm font-semibold group-hover:text-[var(--accent)] transition-colors" style={{ color: 'var(--text)' }}>{project.title}</div><StatusBadge status={status} /></div><p className="text-xs leading-relaxed" style={{ color: 'var(--text-muted)' }}>{project.description}</p></div><ArrowUpRight className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: 'var(--text-faint)' }} aria-hidden="true" /></div></Link>; })}</div>;
