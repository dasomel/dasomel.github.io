...
export function ProjectList({ projects, base, translations }: Props) {
  ...
  const imageFor = (slug: string) => `/images/projects/${visualSlugs.has(slug) ? slug : 'default'}.svg`;
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <label className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border bg-transparent pl-9 pr-3 py-2.5 text-sm outline-none focus:ring-2"
            style={{ borderColor: 'var(--border)', color: 'var(--text)', ['--tw-ring-color' as string]: 'var(--accent-glow)' }}
          />
        </label>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border p-10 text-center" style={{ borderColor: 'var(--border)', color: 'var(--text-muted)' }}>
                  ...
                  <div className="p-5 sm:p-6">
                    <div className="flex items-center justify-between mb-4">
                      <Link href={`${base}/projects/${project.slug}`}>
                        <h3 className="text-lg font-bold hover:text-emerald-400 transition-colors" style={{ color: 'var(--text)' }}>{project.title}</h3>
                      </Link>
