...
export function ProjectList({ projects, base, translations }: Props) {
  ...
  const renderFeatured = (items: Project[]) => <div className="space-y-5 mb-9">{items.map(project => {
    ...
    return <article key={project.slug} className="overflow-hidden rounded-2xl" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--bg)' }}>
  ...
  const renderCompact = (items: Project[]) => <div className="grid sm:grid-cols-2 gap-3">{items.map(project => { const status = resolveStatus(project); return <Link key={project.slug} href={`${base}/projects/${project.slug}`} className="group overflow-hidden rounded-2xl" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--bg)' }}><ProjectVisual src={safeImage(project.slug)} alt="" className="block w-full h-auto" loading="lazy" /><div className="p-4 flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2 mb-1"><div className="text-sm font-semibold group-hover:text-emerald-400 transition-colors" style={{ color: 'var(--text)' }}>{project.title}</div><StatusBadge status={status} /></div><p className="text-xs leading-relaxed" style={{ color: 'var(--text-muted)' }}>{project.description}</p></div><ArrowUpRight className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: 'var(--text-faint)' }} aria-hidden="true" /></div></Link>; })}</div>;
