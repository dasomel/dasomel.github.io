...
1. **생산 차단** — `daily-digest`의 수집 전 **Guard against double publication** 스텝.
   오늘자가 이미 main에 있거나 오늘자 PR이 열려 있으면 그 실행은 아무것도 하지 않는다.
   `workflow_dispatch`의 `force` 입력이 유일한 탈출구다(잘못 만든 다이제스트를 다시 돌릴 때).
2. **머지 차단** — `digest-fallback`은 머지 직전에 `daily-digest-<날짜>.md`가 main에 이미
   있는지 보고, 있으면 **머지하지 않고 경고만 남긴 채 PR을 그대로 둔다.**
   되돌릴 수 없는 행위는 여기서 일어나므로, PR이 어떤 경로로 생겼든 여기서 한 번 더 막힌다.

**`force`로 만든 교체본은 자동 발행되지 않는다 — 사람이 직접 머지해야 한다.** 2번 방어가
의도적으로 그렇게 동작한다. 자동으로 닫지도 않는다(탈출구를 무력화하면 안 되므로).
...
상태가 이상하면 **거부한다**(잘못된 커밋이 놓친 커밋보다 나쁘다): 대상과 다른 브랜치 위, 보강 필드 0건,
원격과 분기, 무관한 파일이 이미 스테이징됨 — 전부 비정상 종료 + 조치 안내. 강제 푸시는 하지 않는다.

**마감은 없다 — 대상을 상태로 정한다.** 브랜치가 살아 있으면 브랜치에, 이미 main 에 발행됐으면
main 에 직접 커밋하고 그 push 가 deploy 를 깨운다. **main 모드가 예외가 아니라 정상 경로다** —
fallback 머지는 06:47~07:53 KST 인데 보강 세션은 08:05 라, 보강이 끝날 땐 브랜치가 거의 항상
이미 머지·삭제돼 있다.

- 2026-08-06~09: 이 경로가 없어서 나흘치 한국어 요약이 생성된 뒤 버려졌다. 08-11 실행은 23건을
  전부 보강해 작업 트리에 두고도 한 건도 발행하지 못했다 — 스크립트가 "이미 발행된 날짜라면
  보강할 것이 없다"며 거부했기 때문이다. **이 게이트를 다시 좁히지 말 것.**
- 2026-07-24~26 에도 연속 3일 유실됐다. 그때는 커밋 단계 자체가 실행되지 않은 경우였다.

**보강 누락은 이제 워크플로가 잡는다.** `digest-health.yml`(00:35 UTC = 09:35 KST, 보강 창 이후)가
`npm run digest:check` 를 돌려 발행본의 `summaryKo` 충족률이 0 이면 `::error::` + 실패한다.
발행을 막지는 않는다 — 보강 없는 발행은 fallback 의 의도된 동작이다. 나흘간 아무도 몰랐던
탐지 공백만 메운다. 마크다운이 JSON 보다 오래된 경우도 같이 잡는다.
**"한국어 포스트에 한글이 있는가"로 검사하지 말 것** — 생성기 푸터가 항상 한국어라 절대 실패하지 않는다.
