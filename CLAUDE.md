...
상태가 이상하면 **거부한다**(잘못된 커밋이 놓친 커밋보다 나쁘다): 다른 브랜치 위, 보강·번역 필드 0건,
...
### 번역은 보강의 하한선이다 (`titleKo`/`excerptKo`)

수집 소스가 전부 영어다. **보강도 번역도 없으면 한국어 포스트가 제목까지 통째로 영어로 나간다** —
fallback 발행일의 글이 그 상태다(2026-07-29 실측: 21건 전부 영어).

번역은 `titleKo`/`excerptKo` 두 필드만 채우고 **플래그가 없다** — 있으면 쓰고 없으면 원문으로 폴백하므로
안 쓸 이유가 없고, 모드를 늘리면 틀릴 여지만 는다. 프롬프트는 `scripts/digest-translate-prompt.md`.
출력량이 보강의 약 1/10 이라 47분 창에 훨씬 잘 들어간다. 여력이 있으면 보강을 우선한다.

- **푸터가 계층을 거짓말하게 두지 말 것.** 생성기는 *실제로 렌더된* 필드를 보고 푸터를 고른다.
  요약이 0건인데 "AI가 요약했습니다"가 붙으면 독자를 속이는 것이다.
- 커밋 메시지도 계층을 따른다 — 번역만이면 `🌐 제목·발췌문 번역`, 요약이 있으면 `✨ AI 요약 보강`.
- **이 계층은 "로컬 태스크가 아예 안 도는" 문제를 고치지 못한다.** 싸고 빨라져 성공 확률이 오를 뿐이다.
  근본 해결은 CI 이전이고 그건 API 키 전제를 깬다 — 다만 번역만이면 하루 출력이 2천 토큰 수준이라
  비용 논거는 보강 때와 다르다. 옮길지는 사람이 정한다.
